...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Alg5_CUSUM17_fixed
% Algorithm 5: CUSUM Two-sided with recursive form p. 17
% Version with a fixed window
...
start = 2;
for k=start+w:length(y)
    % calculate the estimation mu and sigma
    mu(k)= mean(y(k-w:k));
    sigma(k) = std(y(k-w:k))^2;
...
x_len = length(y); % length of (generated) Input signal
...
set(gcf,'name','Baseline 2 with a fixed window','numbertitle','off');

savefig(fig,'baseline2_fixed_2d.fig');

% save experiments results and set-up in mat file
save('baseline2_fixed_2d.mat','iterations','channels_num','segments_num','variance','correlation_value',...
